...
function buildCSPDirective(config, hashes) {
  ...
  for (const [directive, sources] of Object.entries(directives)) {
    ...
    if (directive === "script-src") {
      ...
      if (
        config.csp.externalHashes?.enabled &&
        config.csp.externalHashes.hashes
      ) {
...
function generateHeadersContent(config, hashes) {
  ...
  if (config.generator.comments !== false) {
    ...
    lines.push("#");
    lines.push(
      "# ⚠️  Este archivo es generado automáticamente por scripts/generate-headers.js",
    );
    lines.push(
      "# ⚠️  NO editar manualmente. Modifica config/headers.config.js en su lugar.",
    );
  ...
  if (hasGlobalHeaders) {
    ...
    if (config.features.csp) {
      ...
      if (cspDirective) {
        const cspWithReport =
          config.features.reportUri && config.csp.reportUri
...
async function main() {
  try {
    ...
    if (config.features.inlineHashes && config.csp?.inlineScripts) {
      hashes = processInlineScripts(
        config.csp.inlineScripts,
        config.generator.verbose,
      );
      if (hashes.length > 0) {
    ...
    if (config.features.csp && config.csp.externalHashes?.enabled) {
      log.info(
        `${config.csp.externalHashes.hashes.length} hashes externos configurados`,
      );
    }
    ...
    if (config.features.inlineHashes) {
      const enabledScripts =
        config.csp?.inlineScripts?.filter((s) => s.enabled !== false).length ||
        0;
      console.log(
        `  Scripts inline procesados: ${colors.cyan}${enabledScripts}${colors.reset}`,
      );
      console.log(
        `  Hashes SHA-256 generados: ${colors.cyan}${hashes.length}${colors.reset}`,
      );
    }

    if (config.features.csp && config.csp.externalHashes?.enabled) {
      console.log(
        `  Hashes externos: ${colors.cyan}${config.csp.externalHashes.hashes.length}${colors.reset}`,
      );
    }
    ...
    if (config.features.csp) {
      ...
      console.log(
        `  Dominios whitelistados: ${colors.cyan}${domains}${colors.reset}`,
      );
    }

    if (config.features.cache) {
      const cacheRules = Object.entries(config.cache).filter(
        ([_, v]) => v.enabled !== false,
      ).length;
      console.log(
        `  Reglas de cache: ${colors.cyan}${cacheRules}${colors.reset}`,
      );
    }
